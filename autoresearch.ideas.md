...
- Possible but risky: avoid requiring `schedule.lua` during `setup()` by using a tiny core-local validator or splitting validation into a smaller module. This may cut non-empty schedule startup, but duplicates validation rules unless designed carefully.
