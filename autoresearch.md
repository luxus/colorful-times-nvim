...
- Experiment pending: seed `_parsed_schedule` from setup validation parse results so scheduled cold first apply can reuse work already done by validation. Startup guardrail must stay clearly faster than the minimal reference because this intentionally moves some schedule work into setup.
