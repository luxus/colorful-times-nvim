...
- Kept pending confirmation: exposed `validate_entry` from `schedule_runtime.lua` and re-exported it from `schedule.lua`, removing duplicated validation logic introduced by the runtime split. First run improved `first_apply_delta_us` to `125.260254`, with clean startup/apply/command guardrails. Confirm before stacking.
