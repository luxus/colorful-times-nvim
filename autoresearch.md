...
- Kept/confirmed: skip schedule runtime loading/preprocessing/timers when enabled config has no schedule entries, preserving default behavior while targeting cold first-apply overhead in the empty-schedule scenario. First run improved `first_apply_delta_us` to `111.062012` and collapsed `first_apply_delta_empty_us` from ~150µs to `10.791992`; no-code confirmation measured `first_apply_delta_us=133.396240` and `first_apply_delta_empty_us=0.208008`. Startup guardrail stayed negative and steady-state apply stayed within noise. Keep this fast path.
- Discarded: core-only trusted schedule preprocessing measured `first_apply_delta_us=130.385498`, only ~3µs better than the previous no-code confirmation and well inside `first_apply_delta_iqr_us=82.895264`. Scheduled scenarios moved directionally better, but the aggregate signal was too small and the extra preprocessing path duplicated public validation logic. Keep core using public validated preprocessing; do not retry unchanged unless benchmark precision improves or a larger scheduled-scenario target is adopted.
- Discarded/reverted: seeding `_parsed_schedule` from setup validation parse results looked promising on first run (`first_apply_delta_us=106.291748`) but failed no-code confirmation (`first_apply_delta_us=168.947510`, worse than baseline) and narrowed hourly startup margin to `delta_hourly_us=-11.430664`. Do not move schedule parsing work into setup unless repeated results keep first-apply improved and all startup scenario deltas comfortably negative.
