...
- Benchmark diagnostics kept: paired-delta p25/p75/IQR metrics now report startup/apply noise. First successful run measured `delta_iqr_us=37.452664` and `apply_delta_iqr_us=19.544790`, confirming sub-10µs single-run changes are below noise. First attempt crashed because `collect_apply_pair()` still returned only three values; fixed in the kept retry.
