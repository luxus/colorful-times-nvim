...
- Benchmark diagnostics kept: added `command_p25_us`, `command_p75_us`, and `command_iqr_us` so future command registration tweaks are judged against measured command noise. First run measured `command_us=57.325098`, `command_iqr_us=4.775098`, confirming previous sub-2µs command tweaks were below noise.
