...
- Benchmark diagnostics kept: per-scenario apply delta metrics now complement startup scenario metrics without changing the primary aggregate metric. First run: `apply_delta_empty_us=3.605000`, `apply_delta_two_entry_us=7.482490`, `apply_delta_disabled_us=-1.272090`, `apply_delta_hourly_us=4.993330`; all are within `apply_delta_iqr_us=24.264270`, so apply remains parity/noise-limited.
