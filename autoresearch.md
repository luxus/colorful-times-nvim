...
- Benchmark diagnostics pending: add per-scenario startup delta metrics (`delta_empty_us`, `delta_two_entry_us`, `delta_disabled_us`, `delta_hourly_us`) to detect whether remaining variance or regressions are scenario-specific without changing the primary aggregate metric.
