...
- Kept but noisy: splitting runtime schedule parse/lookup helpers into `schedule_runtime.lua` initially improved `first_apply_delta_us` from `149.531006` to `128.604492`, but no-code confirmation reran worse at `151.708252`. Reverting the split worsened primary further to `181.333008`, so the split remains for now under the first-apply primary, despite added module/validation duplication. Confirm again before stacking changes.
