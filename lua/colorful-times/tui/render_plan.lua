-- lua/colorful-times/tui/render_plan.lua
-- Builds complete TUI render plans without mutating Neovim buffers/windows.
...
local width_fn = function(text) return vim.fn.strdisplaywidth(text) end
...
local function update_layout_metrics(ui)
  ui = ui or { width = MAX_PANEL_WIDTH + 6 }
...
local function display_prefix(text, width)
  ...
  for _, char in each_char(text) do
    local char_width = width_fn(char)
...
local function byte_index_to_display_col(text, byte_index)
  return width_fn(tostring(text or ""):sub(1, math.max(0, (byte_index or 1) - 1)))
end
...
local function fit(text, width)
  ...
  local display_width = width_fn(text)
...
local function frame_title(title, focused)
  ...
  local prefix = "─ " .. label .. " "
  return "  " .. fit(prefix .. string.rep("─", math.max(1, PANEL_WIDTH - width_fn(prefix))), PANEL_WIDTH)
end

local function frame_banner(title, right)
  local prefix = "─ " .. title .. " "
  local suffix = right and (" " .. right .. " ─") or "─"
  local filler = math.max(1, PANEL_WIDTH - width_fn(prefix) - width_fn(suffix))
...
local function timeline(rows, now)
  ...
  TIMELINE_WIDTH = math.max(
    MIN_TIMELINE_WIDTH,
    PANEL_WIDTH - 2 - width_fn(prefix) - width_fn(suffix)
  )
  ...
  return prefix .. table.concat(chars) .. suffix, width_fn(prefix), TIMELINE_WIDTH
end
...
local function theme_width(vm)
  ...
  local function consider(value)
    width = math.max(width, width_fn(view_model.display_theme(value)) + 1)
  end
  ...
  for _, item in ipairs(fields) do
    ...
    if item.key == field then
      ...
      if (item.key == "start" or item.key == "stop") and state.time_input_field == item.key then
        cursor_col = item.col + width_fn(tostring(item.value or "")) + 1
      end
      marks[#marks + 1] = {
        line = lnum,
...
local function footer(lines, marks, state, dirty)
  ...
  if state.mode == ui_state.modes.browse then
    local first = add(lines, "  " .. fit("Tab switch panel  j/k move  <CR> edit  a add  d delete", PANEL_WIDTH))
    local second = add(lines, "  " .. fit("H hold/release  c colors  t toggle  r reload  q quit", PANEL_WIDTH))
...
local function render_lines(input)
  local state = input.state
  update_layout_metrics(input.ui)
  local status = input.status
  local vm = view_model.build(input.config, status, state)
...
function M.build(input)
  width_fn = input.width_fn or width_fn
  local lines, marks, status = render_lines(input)
  return { lines = lines, marks = marks, status = status, window_config = input.window_config }
end
