...
local function render()
	if not (_state.buf and api.nvim_buf_is_valid(_state.buf)) then
		return
	end
	...
	local lines = {}
	-- Status bar
	local status = ct.config.enabled and "  [●] ENABLED  " .. VERSION or "  [○] DISABLED " .. VERSION
	...
	prompt_time("Start time (HH:MM)", existing and existing.start, function(start)
		...
		prompt_time("Stop time (HH:MM)", existing and existing.stop, function(stop)
			...
			pick_colorscheme(existing and existing.colorscheme, function(cs)
				...
				vim.ui.select({ "system", "dark", "light" }, { prompt = "Background: " }, function(bg)
					if not bg then
						cb(nil)
						return
					end
...
local function action_edit()
	...
	if idx < 1 or idx > #ct.config.schedule then
		return
	end
