...
local function detect_linux_auto_background(handle_spawn_result)
	...
	-- Cache Desktop Environment to avoid repeatedly calling shell tests
	if M._cached_linux_de == nil then
		...
		if xdg_current == "KDE" or xdg_session == "KDE" then
			M._cached_linux_de = "KDE"
		elseif xdg_current == "GNOME" or xdg_session == "GNOME" then
			M._cached_linux_de = "GNOME"
		else
			M._cached_linux_de = "UNKNOWN"
			vim.schedule(function()
				vim.notify(
	...
	local de_check = ""
	if M._cached_linux_de == "KDE" then
		de_check = kde_detection
	elseif desktop_env == "GNOME" then
		de_check = gnome_detection
	else
		-- If we can't determine desktop environment natively, default to light
		handle_spawn_result(1)
		return
	end

	-- Try to auto-detect dark mode using the selected desktop environment check
	local handle
	handle = uv.spawn("sh", {
		args = { "-c", de_check },
		stdio = { nil, nil, nil },
