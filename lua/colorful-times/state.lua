...
function M.load()
	...
	if not ok or type(result) ~= "table" then
		vim.notify("colorful-times: failed to parse state file: " .. path, vim.log.levels.WARN)
		return {}
	end
	return result
end
...
function M.save(data)
	...
	if not f then
		vim.notify("colorful-times: could not write state file: " .. path, vim.log.levels.WARN)
		return
	end
