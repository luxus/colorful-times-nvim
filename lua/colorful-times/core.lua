...
local function set_colorscheme(cs, bg)
	...
	if not ok then
		vim.notify("colorful-times: failed to apply colorscheme '" .. cs .. "': " .. err, vim.log.levels.ERROR)
	end
end
...
function M.apply_colorscheme()
	...
	vim.schedule(function()
		set_colorscheme(fallback_cs, fallback)
	end)
	...
	system.get_background(function(detected_bg)
		if detected_bg ~= _previous_bg then
			...
			vim.schedule(function()
				set_colorscheme(real_cs, detected_bg)
			end)
		end
	end, fallback)
end
